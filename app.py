...
def display_title(title: str) -> str:
    """화면 표시용 제목: 앞 괄호 유지, '뒤쪽' 괄호 제거,
    콜론은 '앞에 공백이 있을 때만'(진짜 부제 ' : ') 뒤를 제거.
    예) '페인트 : 이희영 장편소설'      → '페인트'      (공백+콜론=부제)
        '체리새우: 비밀글입니다'        → 그대로 유지     (붙은 콜론=제목 일부)
        '검은 사슴 (한국문학전집 024)'   → '검은 사슴'
    """
    s = strip_html(title)
    s = re.split(r"\s+[:：]\s*", s, maxsplit=1)[0]   # 콜론 앞 공백이 있을 때만 부제로 절단
...
def strip_subtitle(title: str) -> str:
    """매칭용 제목 정리: 괄호 묶음 제거 + '첫 콜론 이후' 제거(짧은 제목으로도 매칭되게).
    예) '체리새우: 비밀글입니다'  → '체리새우'  (입력 '체리새우'와 매칭)
        '페인트 : 이희영 …'       → '페인트'
    """
    s = strip_html(title)
    # 괄호 묶음 제거
    s = re.sub(r"[\(\[（［〔《〈<][^)\]）］〕》〉>]*[\)\]）］〕》〉>]", " ", s)
    # 매칭에서는 콜론(공백 유무 상관없이) 이후를 제거해 원제만 비교
    s = re.split(r"[:：]", s)[0]
